function [ regrets, losses, weights ] = rwma( N, T , eta, nature_type)
...
chosen_expert = randsrc(1, 1, [ 1:1:N ; dist]);
prediction = x(chosen_expert);
...
switch nature_type
    ...
    case 3
        result = funcs.nature.adversarial(1, weights, x);
end

%compare result to all expert predictions
error = (x ~= (ones(1, N) .* result));
%all predictions in error will be scaled by (1-eta)
